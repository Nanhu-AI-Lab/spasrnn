...
namespace {
    ...
    template <typename scalar_t>
    __global__ void s3gd_wR_backward_kernel(
        const torch::PackedTensorAccessor<scalar_t,3,torch::RestrictPtrTraits,size_t> spk_trace_R,
        const torch::PackedTensorAccessor<int64_t,1,torch::RestrictPtrTraits,size_t> aout_b,
        const torch::PackedTensorAccessor<int64_t,1,torch::RestrictPtrTraits,size_t> aout_t,
        const torch::PackedTensorAccessor<int64_t,1,torch::RestrictPtrTraits,size_t> aout_i,
        const torch::PackedTensorAccessor<scalar_t,1,torch::RestrictPtrTraits,size_t> ds_out,
      ...
      if (n < ds_out.size(0)){
          const int b = aout_b[n];  // Batch
          const int t = aout_t[n];  // Timestep
          if (spk_trace_R[b][t][i]==0){
            return;
          }
          const int j = aout_i[n];  // Output neuron index
          const auto val = spk_trace_R[b][t][i] * ds_out[n];
          gpuAtomicAdd(&dW[i][j], val);
      }
    }
///////////////dense2sparse
    template <typename scalar_t>
    __global__ void s3gd_w_backward_Dense2Sparse_kernel(
        const torch::PackedTensorAccessor<scalar_t,3,torch::RestrictPtrTraits,size_t> spk_trace,
        const torch::PackedTensorAccessor<int64_t,1,torch::RestrictPtrTraits,size_t> aout_b,
        const torch::PackedTensorAccessor<int64_t,1,torch::RestrictPtrTraits,size_t> aout_t,
        const torch::PackedTensorAccessor<int64_t,1,torch::RestrictPtrTraits,size_t> aout_i,
    ...
    template <typename scalar_t>
    __global__ void s3gd_wR_backward_Dense2Sparse_kernel(
        const torch::PackedTensorAccessor<scalar_t,3,torch::RestrictPtrTraits,size_t> spk_trace_R,
        const torch::PackedTensorAccessor<int64_t,1,torch::RestrictPtrTraits,size_t> aout_b,
        const torch::PackedTensorAccessor<int64_t,1,torch::RestrictPtrTraits,size_t> aout_t,
        const torch::PackedTensorAccessor<int64_t,1,torch::RestrictPtrTraits,size_t> aout_i,
    ...
    template <typename scalar_t>
    __global__ void s3gd_w_self_R_backward_Dense2Sparse_kernel(
        const torch::PackedTensorAccessor<scalar_t,3,torch::RestrictPtrTraits,size_t> spk_trace_R,
        const torch::PackedTensorAccessor<int64_t,1,torch::RestrictPtrTraits,size_t> aout_b,
        const torch::PackedTensorAccessor<int64_t,1,torch::RestrictPtrTraits,size_t> aout_t,
        const torch::PackedTensorAccessor<int64_t,1,torch::RestrictPtrTraits,size_t> aout_i,
      ...
      if ((i==n) && (n < ds_out.size(0))){
        ...
        //   }
          const int j = aout_i[n];  // Output neuron index
          const auto val = spk_trace_R[b][t][j] * ds_out[n];
          gpuAtomicAdd(&dW[j][j], val); // The spike neuron only affect itself.
      }
    }
...
/////////////dense2sparse/////
    template <typename scalar_t>
    __global__ void s3gd_s_backward_Dense2Sparse_kernel(
        const torch::PackedTensorAccessor<scalar_t,1,torch::RestrictPtrTraits,size_t> ds_out,
        const torch::PackedTensorAccessor<int64_t,1,torch::RestrictPtrTraits,size_t> ts_out,
        const torch::PackedTensorAccessor<int64_t,1,torch::RestrictPtrTraits,size_t> bj_ends,
        const torch::PackedTensorAccessor<int64_t,1,torch::RestrictPtrTraits,size_t> bj,
        const torch::PackedTensorAccessor<scalar_t,1,torch::RestrictPtrTraits,size_t> alphas,
        torch::PackedTensorAccessor<scalar_t,3,torch::RestrictPtrTraits,size_t> ds,
        const int T_Window,
        const int batch_size,
        ...
        if (idx < total_num_threads){
            const int bj_j = bj[idx];
            // b and j
            int b = bj_j / nb_hidden;
            int j = bj_j - b*nb_hidden;

            // Active output tensor start index
            int bj_out = bj_ends[bj_j] ; // Start counting from here back (COMPUTE)
            // int t_out = bj_out==-1 ? -1 : ts_out[bj_out];  // Value of first t_out
            int t_out = ts_out[idx];  // Value of first t_out
            ...
            // Write delta and store indices
            T_Win = (t_out - T_Window)<0? t_out +1: T_Window;
            for(int i = 0; i < T_Win; i++){
                val = ds_out[idx]*alphas[i];
                int a = t_out - i - 1;
                if(a < 0)
                    break;
                //val = weight[i][j]* delta;
                gpuAtomicAdd(&ds[b][a][j], val);
            }
        }
    }
    //////////end///////////
...
torch::Tensor s3gd_wR_backward_cuda(torch::Tensor spk_trace_R,
    ...
    AT_DISPATCH_ALL_TYPES(ds_out.type(), "cuda_torch_test_abs", ([&] {
        s3gd_wR_backward_kernel<scalar_t><<<blocks, threads>>>(
        spk_trace_R.packed_accessor<scalar_t,3,torch::RestrictPtrTraits,size_t>(),
        aout_b.packed_accessor<int64_t,1,torch::RestrictPtrTraits,size_t>(),
        aout_t.packed_accessor<int64_t,1,torch::RestrictPtrTraits,size_t>(),
        aout_i.packed_accessor<int64_t,1,torch::RestrictPtrTraits,size_t>(),
        ds_out.packed_accessor<scalar_t,1,torch::RestrictPtrTraits,size_t>(),
...
torch::Tensor s3gd_w_backward_Dense2Sparse_cuda(torch::Tensor spk_trace,
    ...
    // const dim3 blocks((ds_out.size(0)+threads.x-1) / threads.x, (nb_inputs+threads.y-1)/threads.y) ;
    const dim3 threads = 1024;
    const dim3 blocks((ds_out.size(0)+threads.x-1) / threads.x, nb_inputs) ;

    AT_DISPATCH_ALL_TYPES(ds_out.type(), "cuda_torch_test_abs", ([&] {
        s3gd_w_backward_Dense2Sparse_kernel<scalar_t><<<blocks, threads>>>(
        spk_trace.packed_accessor<scalar_t,3,torch::RestrictPtrTraits,size_t>(),
        aout_b.packed_accessor<int64_t,1,torch::RestrictPtrTraits,size_t>(),
        aout_t.packed_accessor<int64_t,1,torch::RestrictPtrTraits,size_t>(),
        aout_i.packed_accessor<int64_t,1,torch::RestrictPtrTraits,size_t>(),
...
torch::Tensor s3gd_wR_backward_Dense2Sparse_cuda(torch::Tensor spk_trace_R,
    ...
    AT_DISPATCH_ALL_TYPES(ds_out.type(), "cuda_torch_test_abs", ([&] {
        s3gd_wR_backward_Dense2Sparse_kernel<scalar_t><<<blocks, threads>>>(
        spk_trace_R.packed_accessor<scalar_t,3,torch::RestrictPtrTraits,size_t>(),
        aout_b.packed_accessor<int64_t,1,torch::RestrictPtrTraits,size_t>(),
        aout_t.packed_accessor<int64_t,1,torch::RestrictPtrTraits,size_t>(),
        aout_i.packed_accessor<int64_t,1,torch::RestrictPtrTraits,size_t>(),
...
torch::Tensor s3gd_w_self_R_backward_Dense2Sparse_cuda(torch::Tensor spk_trace_R,
    ...
    AT_DISPATCH_ALL_TYPES(ds_out.type(), "cuda_torch_test_abs", ([&] {
        s3gd_w_self_R_backward_Dense2Sparse_kernel<scalar_t><<<blocks, threads>>>(
        spk_trace_R.packed_accessor<scalar_t,3,torch::RestrictPtrTraits,size_t>(),
        aout_b.packed_accessor<int64_t,1,torch::RestrictPtrTraits,size_t>(),
        aout_t.packed_accessor<int64_t,1,torch::RestrictPtrTraits,size_t>(),
        aout_i.packed_accessor<int64_t,1,torch::RestrictPtrTraits,size_t>(),
...
torch::Tensor s3gd_s_backward_Dense2Sparse_cuda(
    ...
    // Create output tensor of shape {batch_size, nb_steps, nb_inputs} filled with zeros
    auto ds = torch::zeros({batch_size, nb_steps, den2spa_output}, torch::TensorOptions().device(torch::kCUDA));

    const int threads = 1024;
    const int blocks = (total_num_threads + threads-1) / threads;

    AT_DISPATCH_ALL_TYPES(ds_out.type(), "cuda_torch_compute_deltas", ([&] {
        s3gd_s_backward_Dense2Sparse_kernel<scalar_t><<<blocks, threads>>>(
        ds_out.packed_accessor<scalar_t,1,torch::RestrictPtrTraits,size_t>(),
        ts_out.packed_accessor<int64_t,1,torch::RestrictPtrTraits,size_t>(),
        bj_ends.packed_accessor<int64_t,1,torch::RestrictPtrTraits,size_t>(),
        bj.packed_accessor<int64_t,1,torch::RestrictPtrTraits,size_t>(),
        alphas.packed_accessor<scalar_t,1,torch::RestrictPtrTraits,size_t>(),

        ds.packed_accessor<scalar_t,3,torch::RestrictPtrTraits,size_t>(),

        T_Window, batch_size, nb_steps, nb_hidden, total_num_threads
        );
    }));
    return ds;
}
